      ...
      snapshot
        ...
        .done(function(response) {
          ...
          if (data.id !== undefined) {
            $("#upload_result").html(
              data.message + ": " + data.id + ", Confidence: " + data.confidence
            );
            // create speech response if the returned data.id is not undefined
            $.post(
              "/speech",
              // ======================Original Phrase===========================
              // { tosay: "Good " + greetingTime(moment()) + " " + data.id },

              { tosay: "This is " + " " + data.id },
              function(response) {
                $("#audio_speech").attr(
                  "src",
